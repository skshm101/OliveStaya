...
public class JwtTokenGeneratorFilter extends OncePerRequestFilter {
	...
	protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
		...
		if (null != authentication) {
			...
			String jwt = Jwts.builder().setIssuer("Saksham").setSubject("JWT Token")
...
//this make sure that this filter will execute only for first time when client call the api /login at first time
	...
	protected boolean shouldNotFilter(HttpServletRequest request) throws ServletException {
		...
		return !path.equals("/olivestays/admins/login") && !path.equals("/olivestays/customers/login")
				&& !path.equals("/olivestays/hotels/login");
	}
